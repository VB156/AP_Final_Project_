...
if (numAgents > 0) {
    ...
    agents.forEach((agent, i) => {
        ...
        // Store the target position on the agent node itself
...
function getTargetY(d) {
    ...
    if (d.type === 'Topic' && d.connections.length > 0) {
        ...
        d.connections.forEach(conn => {
            ...
            const py = vx / mag; // Perpendicular vector Y
            const offset = conn.type === 'input' ? sideOffset : -sideOffset;
            totalY += agent.targetY + py * offset;
        });
        return totalY / d.connections.length;
    }
    return height / 2; // Fallback for unconnected topics
}
...
const simulation = d3.forceSimulation(nodes)
    ...
    .force("x", d3.forceX(d => getTargetX(d)).strength(0.08))
    .force("y", d3.forceY(d => getTargetY(d)).strength(0.08));
